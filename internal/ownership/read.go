...
func (s *Service) ReadResource(caller string, resourcePath string, options ResourceReadOptions) (*ResourceReadResponse, *coreauthz.ErrorResponse) {
	resourcePath = coreauthz.CleanResourcePath(resourcePath)
	if resourcePath == "" {
		return nil, coreauthz.NewErrorResponse("resource_path is required", 400, nil)
	}
	if errResponse := s.RequireOwnershipControl(caller, resourcePath); errResponse != nil {
	...
	rows := []resourceBindingRow{}
	if err := s.DB.Select(&rows, stmt, coreauthz.FormatPathForDb(resourcePath)); err != nil {
		return nil, coreauthz.NewErrorResponse(fmt.Sprintf("ownership resource query failed: %s", err.Error()), 500, &err)
	}

	bindings := make([]ResourceBinding, 0, len(rows))
	for _, row := range rows {
		provenance := map[string]interface{}{}
		if len(row.Provenance) > 0 {
			if err := json.Unmarshal(row.Provenance, &provenance); err != nil {
				return nil, coreauthz.NewErrorResponse(fmt.Sprintf("ownership provenance parse failed: %s", err.Error()), 500, &err)
			}
		}
		...
		bindings = append(bindings, ResourceBinding{
			ResourcePath: coreauthz.FormatDbPath(row.ResourcePath),
...
func includeOwnershipBinding(row resourceBindingRow, provenance map[string]interface{}, options ResourceReadOptions) bool {
...
func isAdminOwnershipBinding(row resourceBindingRow, provenance map[string]interface{}) bool {
	if row.Kind == "admin" {
		return true
	}
