...
func (role *Role) UnmarshalJSON(data []byte) error {
	...
	err = coreauthz.ValidateJSON("role", role, fields, optionalFields)
	if err != nil {
		return err
	}
...
func (role *Role) validate() *coreauthz.ErrorResponse {
	if len(role.Permissions) == 0 {
		return coreauthz.NewErrorResponse("role has no permissions", 400, nil)
	}
	return nil
}
...
func (roleFromQuery *RoleFromQuery) Standardize() Role {
	return roleFromQuery.standardize()
}
...
func ListRolesFromDb(db *sqlx.DB) ([]RoleFromQuery, error) {
	return listRolesFromDb(db)
}

func RolesWithNames(db *sqlx.DB, roleNames []string) ([]RoleFromQuery, error) {
	return rolesWithNames(db, roleNames)
}

func RoleWithName(db *sqlx.DB, name string) (*RoleFromQuery, error) {
	return roleWithName(db, name)
}

func (role *Role) createInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	...
	stmt = coreauthz.MultiInsertStmt(permissionTable, len(role.Permissions))
...
func (role *Role) CreateInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	return role.createInDb(db)
}

func (role *Role) overwriteInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	errResponse := role.validate()
	if errResponse != nil {
		return errResponse
	}
...
func (role *Role) OverwriteInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	return role.overwriteInDb(db)
}

func (role *Role) deleteInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
