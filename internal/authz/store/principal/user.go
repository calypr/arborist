...
func (user *User) UnmarshalJSON(data []byte) error {
	...
	err = coreauthz.ValidateJSON("user", user, fields, optionalFields)
	if err != nil {
		return err
	}
...
func (userFromQuery *UserFromQuery) Standardize() User {
	return userFromQuery.standardize()
}
...
func FetchUserPolicyInfo(db *sqlx.DB, user_name string, policy_name string) (*UserPolicyInfoFromQuery, error) {
	return fetchUserPolicyInfo(db, user_name, policy_name)
}
...
func ListUsersFromDb(db *sqlx.DB) ([]UserFromQuery, error) {
	return listUsersFromDb(db)
}

func UserWithName(db *sqlx.DB, name string) (*UserFromQuery, error) {
	return userWithName(db, name)
}

func (user *User) createInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	tx, err := db.Beginx()
	if err != nil {
...
func (user *User) CreateInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	return user.createInDb(db)
}

func (user *User) updateInDb(db *sqlx.DB, name *string, email *string) *coreauthz.ErrorResponse {
	stmt := `
		UPDATE usr
		SET
...
func (user *User) UpdateInDb(db *sqlx.DB, name *string, email *string) *coreauthz.ErrorResponse {
	return user.updateInDb(db, name, email)
}

func (user *User) deleteInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
...
func (user *User) DeleteInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	return user.deleteInDb(db)
}

func grantUserPolicy(db *sqlx.DB, username string, policyName string, expiresAt *time.Time, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	stmt := `
...
func GrantUserPolicy(db *sqlx.DB, username string, policyName string, expiresAt *time.Time, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return grantUserPolicy(db, username, policyName, expiresAt, authzProvider)
}

func revokeUserPolicy(db *sqlx.DB, username string, policyName string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	stmt := `
...
func RevokeUserPolicy(db *sqlx.DB, username string, policyName string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return revokeUserPolicy(db, username, policyName, authzProvider)
}

func revokeUserPolicyAll(db *sqlx.DB, username string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	stmt := `
		DELETE FROM usr_policy
		WHERE usr_id = (SELECT id FROM usr WHERE name = $1)
	`
...
func RevokeUserPolicyAll(db *sqlx.DB, username string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return revokeUserPolicyAll(db, username, authzProvider)
}

func addUserToGroup(db *sqlx.DB, username string, groupName string, expiresAt *time.Time, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	if groupName == coreauthz.AnonymousGroup || groupName == coreauthz.LoggedInGroup {
		return coreauthz.NewErrorResponse("can't add users to built-in groups", 400, nil)
	}
	if username == "" {
		return coreauthz.NewErrorResponse("missing `username` argument", 400, nil)
	}
	stmt := `
	...
	if err != nil {
		...
		var groupExists int
		err = db.QueryRow(`SELECT 1 FROM grp WHERE name = $1`, groupName).Scan(&groupExists)
...
func AddUserToGroup(db *sqlx.DB, username string, groupName string, expiresAt *time.Time, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return addUserToGroup(db, username, groupName, expiresAt, authzProvider)
}

func removeUserFromGroup(db *sqlx.DB, username string, groupName string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	stmt := `
		DELETE FROM usr_grp
		WHERE usr_id = (SELECT id FROM usr WHERE name = $1)
...
func RemoveUserFromGroup(db *sqlx.DB, username string, groupName string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return removeUserFromGroup(db, username, groupName, authzProvider)
}
