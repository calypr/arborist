package principal

import (
	"database/sql"
	"fmt"

	coreauthz "github.com/calypr/arborist/internal/authz/core"
	"github.com/calypr/arborist/internal/authz/store/policy"
	"github.com/jmoiron/sqlx"
	"github.com/lib/pq"
)

type Client struct {
	ClientID string   `json:"clientID"`
	Policies []string `json:"policies"`
}

type ClientFromQuery struct {
	ClientID string         `db:"external_client_id"`
	Policies pq.StringArray `db:"policies"`
}

func (clientFromQuery *ClientFromQuery) standardize() Client {
	client := Client{
		ClientID: clientFromQuery.ClientID,
		Policies: clientFromQuery.Policies,
	}
	return client
}

func (clientFromQuery *ClientFromQuery) Standardize() Client {
	return clientFromQuery.standardize()
}

func clientWithClientID(db *sqlx.DB, clientID string) (*ClientFromQuery, error) {
	stmt := `
		SELECT
			client.external_client_id,
			array_remove(array_agg(DISTINCT policy.name), NULL) AS policies
		FROM client
		LEFT JOIN client_policy ON client.id = client_policy.client_id
		LEFT JOIN policy ON policy.id = client_policy.policy_id
		WHERE client.external_client_id = $1
		GROUP BY client.id
		LIMIT 1
	`
	clients := []ClientFromQuery{}
	err := db.Select(&clients, stmt, clientID)
	if len(clients) == 0 {
		return nil, nil
	}
	if err != nil {
		return nil, err
	}
	client := clients[0]
	return &client, nil
}

func listClientsFromDb(db *sqlx.DB) ([]ClientFromQuery, error) {
	stmt := `
		SELECT
			client.external_client_id,
			array_remove(array_agg(DISTINCT policy.name), NULL) AS policies
		FROM client
		LEFT JOIN client_policy ON client.id = client_policy.client_id
		LEFT JOIN policy ON policy.id = client_policy.policy_id
		GROUP BY client.id
	`
	clients := []ClientFromQuery{}
	err := db.Select(&clients, stmt)
	if err != nil {
		return nil, err
	}
	return clients, nil
}

func ListClientsFromDb(db *sqlx.DB) ([]ClientFromQuery, error) {
	return listClientsFromDb(db)
}

func ClientWithClientID(db *sqlx.DB, clientID string) (*ClientFromQuery, error) {
	return clientWithClientID(db, clientID)
}

func (client *Client) createInDb(db *sqlx.DB, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	tx, err := db.Beginx()
	if err != nil {
		msg := fmt.Sprintf("couldn't open database transaction: %s", err.Error())
		return coreauthz.NewErrorResponse(msg, 500, &err)
	}

	var clientDBID int
	stmt := `
		INSERT INTO client(external_client_id)
		VALUES ($1)
		RETURNING id
	`
	row := tx.QueryRowx(stmt, client.ClientID)
	err = row.Scan(&clientDBID)
	if err != nil {
		// should add more checking here to guarantee the correct error
		_ = tx.Rollback()
		// this should only fail because the client was not unique. return error
		// accordingly
		msg := fmt.Sprintf("failed to insert client: client with this ID already exists: %s", client.ClientID)
		return coreauthz.NewErrorResponse(msg, 409, &err)
	}

	if len(client.Policies) > 0 {
		remaining := make(map[string]interface{})
		for _, policy := range client.Policies {
			remaining[policy] = nil
		}

		stmt = `
			WITH policies AS (
				SELECT id, name FROM policy WHERE name = ANY($1)
			),
			client_policies AS (
				INSERT INTO client_policy (client_id, policy_id, authz_provider)
				SELECT $2, id, $3 FROM policies
			)
			SELECT name FROM policies
		`
		rows, err := tx.Query(stmt, pq.Array(client.Policies), clientDBID, authzProvider)
		if err != nil {
			_ = tx.Rollback()
			return coreauthz.NewErrorResponse("failed to grant policy", 500, &err)
		}
		for rows.Next() {
			var policy string
			err = rows.Scan(&policy)
			if err != nil {
				_ = tx.Rollback()
				return coreauthz.NewErrorResponse("failed to grant policy", 500, &err)
			}
			delete(remaining, policy)
		}
		if len(remaining) != 0 {
			_ = tx.Rollback()
			missingPolicies := make([]string, len(remaining))
			i := 0
			for policy := range remaining {
				missingPolicies[i] = policy
				i++
			}
			msg := fmt.Sprintf(
				"failed to grant policy to client: policies does not exist: %v",
				missingPolicies,
			)
			return coreauthz.NewErrorResponse(msg, 404, &err)
		}
	}

	err = tx.Commit()
	if err != nil {
		_ = tx.Rollback()
		msg := fmt.Sprintf("couldn't commit database transaction: %s", err.Error())
		return coreauthz.NewErrorResponse(msg, 500, &err)
	}

	return nil
}

func (client *Client) CreateInDb(db *sqlx.DB, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return client.createInDb(db, authzProvider)
}

func (client *Client) deleteInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	stmt := "DELETE FROM client WHERE external_client_id = $1"
	_, err := db.Exec(stmt, client.ClientID)
	if err != nil {
		// TODO: verify correct error
		msg := fmt.Sprintf("failed to delete client: client does not exist: %s", client.ClientID)
		return coreauthz.NewErrorResponse(msg, 404, nil)
	}
	return nil
}

func (client *Client) DeleteInDb(db *sqlx.DB) *coreauthz.ErrorResponse {
	return client.deleteInDb(db)
}

func grantClientPolicy(db *sqlx.DB, clientID string, policyName string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	stmt := `
		INSERT INTO client_policy(client_id, policy_id, authz_provider)
		VALUES ((SELECT id FROM client WHERE external_client_id = $1), (SELECT id FROM policy WHERE name = $2), $3)
	`
	_, err := db.Exec(stmt, clientID, policyName, authzProvider)
	if err != nil {
		client, err := clientWithClientID(db, clientID)
		if client == nil {
			msg := fmt.Sprintf(
				"failed to grant policy to client: client does not exist: %s",
				clientID,
			)
			return coreauthz.NewErrorResponse(msg, 404, nil)
		}
		if err != nil {
			msg := "client query failed"
			return coreauthz.NewErrorResponse(msg, 500, &err)
		}
		policy, err := policy.PolicyWithName(db, policyName)
		if policy == nil {
			msg := fmt.Sprintf(
				"failed to grant policy to client: policy does not exist: %s",
				policyName,
			)
			return coreauthz.NewErrorResponse(msg, 404, nil)
		}
		if err != nil {
			msg := "policy query failed"
			return coreauthz.NewErrorResponse(msg, 500, &err)
		}
		// at this point, we assume the client already has this policy. this is fine.
	}
	return nil
}

func GrantClientPolicy(db *sqlx.DB, clientID string, policyName string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return grantClientPolicy(db, clientID, policyName, authzProvider)
}

func revokeClientPolicy(db *sqlx.DB, clientID string, policyName string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	stmt := `
		DELETE FROM client_policy
		WHERE client_id = (SELECT id FROM client WHERE external_client_id = $1)
		AND policy_id = (SELECT id FROM policy WHERE name = $2)
	`
	var err error = nil
	if authzProvider.Valid {
		stmt += " AND authz_provider = $3"
		_, err = db.Exec(stmt, clientID, policyName, authzProvider)
	} else {
		_, err = db.Exec(stmt, clientID, policyName)
	}
	if err != nil {
		msg := "revoke policy query failed"
		return coreauthz.NewErrorResponse(msg, 500, &err)
	}
	return nil
}

func RevokeClientPolicy(db *sqlx.DB, clientID string, policyName string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return revokeClientPolicy(db, clientID, policyName, authzProvider)
}

func revokeClientPolicyAll(db *sqlx.DB, clientID string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	stmt := `
		DELETE FROM client_policy
		WHERE client_id = (SELECT id FROM client WHERE external_client_id = $1)
	`
	var err error = nil
	if authzProvider.Valid {
		stmt += " AND authz_provider = $2"
		_, err = db.Exec(stmt, clientID, authzProvider)
	} else {
		_, err = db.Exec(stmt, clientID)
	}
	if err != nil {
		msg := "revoke policy query failed"
		return coreauthz.NewErrorResponse(msg, 500, &err)
	}
	return nil
}

func RevokeClientPolicyAll(db *sqlx.DB, clientID string, authzProvider sql.NullString) *coreauthz.ErrorResponse {
	return revokeClientPolicyAll(db, clientID, authzProvider)
}
