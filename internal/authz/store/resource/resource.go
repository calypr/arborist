...
func (resource *ResourceIn) UnmarshalJSON(data []byte) error {
	...
	errPath := coreauthz.ValidateJSON("resource", resource, fields, optionalFieldsPath)
	...
	errName := coreauthz.ValidateJSON("resource", resource, fields, optionalFieldsName)
...
func (resourceFromQuery *ResourceFromQuery) Standardize() ResourceOut {
	return resourceFromQuery.standardize()
}
...
func ResourceWithPath(db *sqlx.DB, path string) (*ResourceFromQuery, error) {
	return resourceWithPath(db, path)
}
...
func (resource *ResourceIn) createInDb(tx *sqlx.Tx) *coreauthz.ErrorResponse {
...
func (resource *ResourceIn) createRecursively(tx *sqlx.Tx) *coreauthz.ErrorResponse {
...
func (resource *ResourceIn) CreateRecursively(tx *sqlx.Tx) *coreauthz.ErrorResponse {
	return resource.createRecursively(tx)
}

func (resource *ResourceIn) deleteInDb(tx *sqlx.Tx) *coreauthz.ErrorResponse {
	if resource.Path == "" {
		msg := "resource missing required field `path`"
		return coreauthz.NewErrorResponse(msg, 400, nil)
	}
	protected, errResponse := protection.ResourceHasProtectedOwnership(tx, resource.Path)
	if errResponse != nil {
		return errResponse
	}
	if protected {
...
func (resource *ResourceIn) addPath(parent string) *coreauthz.ErrorResponse {
	if resource.Path == "" {
		if resource.Name == "" {
			err := coreauthz.MissingRequiredField("resource", "name")
			errResponse := coreauthz.NewErrorResponse(err.Error(), 400, &err)
...
func (resource *ResourceIn) AddPath(parent string) *coreauthz.ErrorResponse {
	return resource.addPath(parent)
}

func (resource *ResourceIn) updateInDb(tx *sqlx.Tx, merge bool) *coreauthz.ErrorResponse {
	// arborist uses `/` for path separator; ltree in postgres uses `.`
	path := FormatPathForDb(resource.Path)
...
func (resource *ResourceIn) UpdateInDb(tx *sqlx.Tx, merge bool) *coreauthz.ErrorResponse {
	return resource.updateInDb(tx, merge)
}
