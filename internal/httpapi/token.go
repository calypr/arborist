...
func (server *Server) decodeToken(token string, scopes []string) (*coreauthz.TokenInfo, error) {
...
func (server *Server) usernameFromBearer(r *http.Request) (string, *coreauthz.ErrorResponse) {
	...
	if err != nil {
		return "", coreauthz.NewErrorResponse(fmt.Sprintf("invalid authorization token: %s", err.Error()), http.StatusUnauthorized, &err)
	}
	if info.Username == "" {
		return "", coreauthz.NewErrorResponse("authorization token does not identify a user", http.StatusUnauthorized, nil)
	}
